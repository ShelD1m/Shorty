...
async function apply(){
    ...
    if(a.length<6){ toast("Минимум 6 символов"); return; }
    if(a!==b){ toast("Пароли не совпадают"); return; }
    el.btn.disabled=true;
    try{
        await api("/api/auth/password/reset",{method:"POST",body:{token, newPassword:a}});
        el.msg.textContent="Пароль обновлён. Теперь войдите с новым паролем.";
        toast("Готово");
        setTimeout(()=>location.href="login.html",1200);
    }catch(e){
        el.msg.textContent="Ошибка: "+String(e.message||"не удалось обновить");
        toast(String(e.message||"Ошибка"));
    }finally{ el.btn.disabled=false; }
}

(function init(){
    el.btn.addEventListener("click", apply);
    el.p2.addEventListener("keydown",(e)=>{ if(e.key==="Enter") apply(); });
})();
